...
**Versioning.** `schema` says which format the files are in; vulners.nse
refuses
...
publish and is the whole update protocol: a cached copy fetches the small
index,
...
def next_serial():
    ...
    A plain counter: the only question a cached copy asks is "is there
    something newer than what I have", and an integer answers it without a
    clock, a timezone or a tie-break.
    """
    if not INDEX.exists():
        return 1
    try:
...
def write_index():
    ...
    for kind, (filename, key) in sorted(DICTIONARIES.items()):
        ...
        if payload.get("schema") != SCHEMA:
            raise SystemExit("catalog/%s declares schema %r, this tool "
                "writes %d"
                             % (filename, payload.get("schema"), SCHEMA))
        entries[kind] = {"file": filename,
                         "entries": len(payload.get(key) or {})}
    ...
    print("catalog/index.json  schema %d, serial %d" % (SCHEMA,
                                                        index["serial"]))
...
def check():
    ...
    if not INDEX.exists():
        raise SystemExit("catalog/index.json is missing - run "
            "tools/catalog.py --index")
    index = load(INDEX)
    ...
    for kind, (filename, key) in DICTIONARIES.items():
        ...
        payload = load(target)
        if payload.get("schema") != SCHEMA:
            problems.append("%s has schema %r" % (filename,
                                                  payload.get("schema")))
    ...
    for path in paths:
        if not isinstance(path, str) or not path.startswith("/") \
            ...
            problems.append("path %r is not a request line this script would "
                "send"
                            % (path,))
    ...
    for probe in bodies.get("probes") or []:
        ...
        if not ALIAS.match(probe.get("alias", "")):
            problems.append("probe %r has alias %r" % (label,
                                                       probe.get("alias")))
        for rule in probe.get("detect") or []:
            if not isinstance(rule, dict) or not isinstance(rule.get("regex"),
                                                                     str):
                problems.append("probe %r has a detector whose regex is not a "
                                "string; string.find raises on it, and a "
                                    "raising "
        ...
        for rule in probe.get("extract") or []:
            if not isinstance(rule, dict) or not isinstance(rule.get("regex"),
                                                                     str):
                problems.append("probe %r has an extractor whose regex is "
                    "not a "
                                "string" % label)
        for path in probe.get("paths") or []:
            if not isinstance(path, str) or not path.startswith("/"):
                problems.append("probe %r would request %r" % (label, path))
        if not (probe.get("detect") and probe.get("extract")
                and probe.get("paths")):
            problems.append("probe %r cannot be triggered, sent or "
                "read" % label)
